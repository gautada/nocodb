#!/bin/sh
#
# Health check: verifies the running NocoDB version matches the latest
...
if [ -z "$CURRENT_VERSION" ]; then
  echo "appversion-check: failed to get running NocoDB version"
  exit 1
fi

LATEST_VERSION=$(/usr/bin/container-latest)
if [ -z "$LATEST_VERSION" ]; then
  echo "appversion-check: failed to get latest NocoDB release version"
  exit 1
fi
